...
int util_socket_and_bind(struct server *srv)
{
    ...
    if (!bound)
    {
        close(fd);
#ifdef DEBUG
        printf("No se ha podido vincular a ninguna direccion\n");
#endif
        return -1;
    }
    ...
    {
#ifdef DEBUG
        printf("Fallo al poner el socket en modo no bloqueante. Esto tendra serias implicaciones en el rendimiento\n");
#endif
    }
    return fd;
}
